...
{
    // SwitchU-style gradient background: near-black navy → deep purple-blue
    {
        NVGpaint bg = nvgLinearGradient(vg,
            static_cast<float>(x), static_cast<float>(y),
            static_cast<float>(x), static_cast<float>(y + height),
            nvgRGB(10, 10, 20),   // #0A0A14 top
            nvgRGB(20, 15, 40));  // #140F28 bottom
        nvgBeginPath(vg);
...
std::string AppletFrame::getTitle()
{
    return this->title;
}
