...
void ScrollView::scrollBy(float delta)
{
    ...
    float contentHeight = (float)this->contentView->getHeight();
    float viewHeight = (float)this->height;
    ...
    float newScroll = this->scrollY + delta;

    // Calculate maximum scroll to prevent content from scrolling beyond bottom
    // When scrollY = 0.0, content top is at view top
    // When scrollY = maxScroll, content bottom is at view bottom
    float maxScroll = (contentHeight - viewHeight) / contentHeight;
...
void ScrollView::processScroll(const ScrollDirection direction)
{
    // Calculate scroll delta based on direction
    // Adjust this value to change scroll speed (smaller = faster, larger = slower)
    float scrollStep = 0.05f;

    switch (direction) {
        case ScrollDirection::UP:
            // Swipe UP = content moves UP = scroll forward (down)
            scrollBy(scrollStep);
            break;
        case ScrollDirection::DOWN:
            // Swipe DOWN = content moves DOWN = scroll backward (up)
            scrollBy(-scrollStep);
            break;
        default:
            break;
