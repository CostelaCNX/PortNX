...
void ListItem::click(View* sender) {
    if (this->isEnabled()) {
        this->setState(false);
...
void ToggleListItem::click(View* sender) {
    if (this->isEnabled()) {
        this->setState(false);
        this->onClick();
        this->setState(true);
...
{
    this->result = Swkbd::openForText([&](std::string text) {
...
void InputListItem::click(View* sender) {
    if (this->isEnabled()) {
        this->setState(false);
        this->onClick();
        this->setState(true);
...
void IntegerInputListItem::click(View* sender) {
    if (this->isEnabled()) {
        this->setState(false);
        this->onClick();
        this->setState(true);
...
void List::addView(View* view, bool fill)
{
    ...
    // Automatically register ListItems as touchable objects
    // so they can be clicked via touch input
    if (dynamic_cast<ListItem*>(view) != nullptr) {
        // Check if already in touchableObjects (avoid duplicates)
        std::vector<View*> touchables = this->getTouchableObjects();
        bool alreadyAdded = false;
        for (View* obj : touchables) {
            if (obj == view) {
                alreadyAdded = true;
                break;
            }
...
void List::setScrollOffset(long int offset)
{
    this->layout->setScrollOffset(offset);
}

void List::setFirstItem(long int index)
{
    this->layout->setFirstItem(index);
}

int List::getFirstItem()
{
    return this->layout->getFirstItem();
}

long int List::getSelectedItemIndex()
{
    if (getViewsCount() > 0)
        return this->layout->getSelectedItemIndex();
	else
		return -1;
}
