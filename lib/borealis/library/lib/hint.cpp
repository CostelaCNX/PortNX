...
{
    // RSTICK is right after + unless LSTICK is visible
    if (a.key == Key::LSTICK)
        return true;

    // LSTICK is always immediately after +
    if (a.key == Key::RSTICK)
        return true;
    ...
    // PLUS is the first icon from Lright to left
    if (b.key == Key::PLUS)
        return true;

    // A is after all others but PLUS
    if (b.key == Key::A && a.key != Key::PLUS)
        return true;
