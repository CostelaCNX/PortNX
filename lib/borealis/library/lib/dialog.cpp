...
void DialogButton::click(View* sender) {
    if (this->isEnabled()) {
//        forceWriteLog("dialog.cpp", "click() - Clicked button OK");
        this->setState(false);
...
GenericEvent* DialogButton::getClickEvent()
{
    return &this->clickEvent;
}

void DialogButton::draw(NVGcontext* vg, int x, int y, unsigned width, unsigned height, Style* style, FrameContext* ctx)
{
}
...
//        forceWriteLog("dialog.cpp", fmt::format("open() - blocking inputs! - count: '{}'", Application::getBlockInputsTokens()));
    }
}
...
void Dialog::close(std::function<void(void)> cb)
{
    ...
    if (this->buttons.size() == 0) {
//        forceWriteLog("dialog.cpp", "open() - unblocking inputs!");
...
void Dialog::layout(NVGcontext* vg, Style* style, FontStash* stash)
{
    ...
    if (this->verticalButtonsLayout)
    {
        ...
        {
            this->buttons[0]->setX(this->frameX + 164);
            this->buttons[0]->setY(this->frameY + this->frameHeight - buttonsHeight);
...
void Dialog::rebuildButtons()
{
    ...
    if (this->buttons.size() > 0)
    {
        ...
        if (this->buttons.size() == 1)
        {
            ...
            this->addTouchableObjects(button);
        }
        // Two buttons on one row
        else if (this->buttons.size() == 2)
        {
...
void Dialog::onWindowFocus() {
    //checking whether there's more than one window in pushed to the user's view and setting the correct touchable object
//    forceWriteLog("dialog.cpp", fmt::format("onWindowFocus() - View Stack Size: '{}'", Application::getViewStackSize()));

    this->mainView = Application::getMainView();
    this->currentTouchableView = Application::getCurrentTouchableView();

    if (Application::getViewStackSize() > 1) {
//        forceWriteLog("dialog.cpp", "onWindowFocus() - Size bigger than 1");
        Application::setMainView(this);
        Application::setCurrentTouchableView(nullptr);

//        forceWriteLog("dialog.cpp", "onWindowFocus() - Setting current touchable view to 'nullptr'");
//        forceWriteLog("dialog.cpp", fmt::format("onWindowFocus() - Setting Main View to '{}'", Application::getMainView()->describe()));
    }
    else {
//        forceWriteLog("dialog.cpp", "onWindowFocus() - Size less than or equal to 1");
        Application::setMainView(nullptr);
        Application::setCurrentTouchableView(this);

//        forceWriteLog("dialog.cpp", fmt::format("onWindowFocus() - Setting current touchable view to '{}'", Application::getCurrentTouchableView()->describe()));
//        forceWriteLog("dialog.cpp", "onWindowFocus() - Setting Main View to 'nullptr'");
    }
}

Dialog::~Dialog()
{
    Application::setMainView(this->mainView);
    Application::setCurrentTouchableView(this->currentTouchableView);
