...
void StagedAppletFrame::firstStage()
{
    enterStage((int)0, true);
}
...
void StagedAppletFrame::lastStage()
{
    enterStage((int)stageViews.size() - 1, true);
}

void StagedAppletFrame::stageAt(int index) {
    enterStage((int)index, true);
}

int StagedAppletFrame::getStageIndex() {
    return currentStage;
}

void StagedAppletFrame::processTouch(u32 x, u32 y) {
//    forceWriteLog("staged_applet_frame.cpp", "Method StagedAppletFrame::processTouch() started");

    for (brls::View* obj : stageViews[currentStage]->getTouchableObjects()) {
//        forceWriteLog("staged_applet_frame.cpp", fmt::format("StagedAppletFrame::processTouch() - obj: '{}'", obj->describe()));
	    if (obj->IsPositionInBounds(x, y)) {
            //focusing the clicked object first
            if (obj->isClickFocusable()) {
                Application::giveFocus(obj);
//                forceWriteLog("sidebar.cpp", fmt::format("click() - gave focus to obj: '{}' - label: '{}'", obj->describe(), dynamic_cast<SidebarItem*>(obj)->getLabel()));
            }
//            else
//                forceWriteLog("sidebar.cpp", fmt::format("click() - skipped giving focus to obj: '{}' - label: '{}'", obj->describe(), dynamic_cast<SidebarItem*>(obj)->getLabel()));

            obj->click(obj);
            break;
        }
    }
//    forceWriteLog("staged_applet_frame.cpp", "Method StagedAppletFrame::processTouch() ended");
}
...
void StagedAppletFrame::enterStage(int index, bool requestFocus)
{
    ...
    //calling onWindowFocusLost() for previous stage
    if (oldStage > 0)
        stageViews[currentStage]->onWindowFocusLost();
    ...
    if (this->hasContentView()) {
        brls::Application::setCurrentTouchableView(stageViews[currentStage]);
//        forceWriteLog("staged_applet_frame.cpp", fmt::format("enterStage() - Setting current touchable view to '{}'", stageViews[currentStage]->describe()));

        //calling onWindowFocus() for current stage
        stageViews[currentStage]->onWindowFocus();
    }
}
...
void StagedAppletFrame::onWindowFocus() {
//    if (Application::getCurrentTouchableView() != nullptr)
//        forceWriteLog("staged_applet_frame.cpp", fmt::format("onWindowFocus() - Current touchable view: '{}'", Application::getCurrentTouchableView()->describe()));
//    else
//        forceWriteLog("staged_applet_frame.cpp", "onWindowFocus() - Current touchable view is nullptr");
//    if (Application::getMainView() != nullptr)
//        forceWriteLog("staged_applet_frame.cpp", fmt::format("onWindowFocus() - Current main view: '{}'", Application::getMainView()->describe()));
//    else
//        forceWriteLog("staged_applet_frame.cpp", "onWindowFocus() - Current main view is nullptr");

    AppletFrame::onWindowFocus();
}
