...
{
    if (auto it = std::find(this->actions.begin(), this->actions.end(), key); it != this->actions.end())
        this->actions.erase(std::find(this->actions.begin(), this->actions.end(), key));
}
...
void View::setX(int posX)
{
    this->x = posX;
}

void View::setY(int posY)
{
    this->y = posY;
}
...
void View::onWindowFocus() {
    if (this->touchable) {
//        forceWriteLog("view.cpp", fmt::format("View::onWindowFocus() - setting currentTouchableView to '{}'", this->describe()));
        Application::setCurrentTouchableView(this);
    }
}

void View::onWindowFocusLost() {
}

void View::processTouch(u32 x, u32 y) {
    forceWriteLog("view.cpp", fmt::format("processTouch() START - view: '{}' - touch: ({}, {})", this->describe(), x, y));

    std::vector<View*> touchables = this->getTouchableObjects();
    forceWriteLog("view.cpp", fmt::format("  Touchable objects count: {}", touchables.size()));

    for (brls::View* obj : touchables) {
        // Safety check: skip null objects
        if (obj == nullptr) {
            forceWriteLog("view.cpp", "  Skipping NULL object");
            continue;
        }

        forceWriteLog("view.cpp", fmt::format("  Checking obj: '{}' - bounds: ({}, {}, {}, {})",
            obj->describe(), obj->getX(), obj->getY(), obj->getWidth(), obj->getHeight()));

        // Check if touch is within this object's bounds
	    if (obj->IsPositionInBounds(x, y)) {
            forceWriteLog("view.cpp", fmt::format("  HIT! Object: '{}' - clickFocusable: {}",
                obj->describe(), obj->isClickFocusable()));

            // Selective focus behavior:
            // - SidebarItem: Give focus (needed for tab switching)
            // - ListItem: Don't give focus (prevents wrong item focus)
            // - Other clickable items: Give focus if clickFocusable
            std::string objType = obj->describe();
            bool isSidebarItem = (objType.find("SidebarItem") != std::string::npos);
            bool isListItem = (objType.find("ListItem") != std::string::npos || objType.find("N4brls8ListItemE") != std::string::npos);

            if (isSidebarItem || (!isListItem && obj->isClickFocusable())) {
                Application::giveFocus(obj);
                forceWriteLog("view.cpp", fmt::format("  Gave focus to: '{}'", obj->describe()));
            } else {
                forceWriteLog("view.cpp", "  Skipped focus (ListItem or not clickFocusable)");
            }

            // Trigger click event
		    obj->click(obj);
            forceWriteLog("view.cpp", fmt::format("  Clicked: '{}'", obj->describe()));
            break; // Only process one object per touch
        }
    }
    forceWriteLog("view.cpp", "processTouch() END");
}

void View::processScroll(const ScrollDirection scroll) {
}

void View::addTouchableObjects(View* touchableObject)
{
    touchableObjects.push_back(touchableObject);
}

int View::getTouchableObjectsCount()
{
    return touchableObjects.size();
}

std::vector<View*> View::getTouchableObjects()
{
    return touchableObjects;
}

void View::clearTouchableObjects()
{
    touchableObjects.clear();
}

bool View::IsPositionInBounds(u32 x, u32 y) {
    bool res = (int)x >= this->x && (int)y >= this->y && x < (this->x + this->width) && y < (this->y + this->height);

//    forceWriteLog("view.cpp", fmt::format("IsPositionInBounds() - object: '{}' - parameters: [touch.x='{}'] - [touch.y='{}'] - [obj.x='{}'] - [obj.y='{}'] - [obj.width='{}'] - [obj.height='{}'] - [obj.x+obj.width='{}'] - [obj.y+obj.height='{}'] - return='{}'", this->describe(), x, y, this->x, this->y, this->width, this->height, (this->x+this->width), (this->y+this->height), res));

    return res;
}

void View::click(View* sender) {
}

void View::setState(bool state) {
    this->enabled = state;
}
...
void View::setClickFocusable(bool focusable) {
    this->clickFocusable = focusable;
}

bool View::isClickFocusable() {
    return this->clickFocusable;
}

void View::setFocused(bool focusState) {
    this->focused = focusState;
}

void View::wasAlreadyFirstFocused(bool firstFocused) {
    this->alreadyFirstFocused = firstFocused;
}

bool View::isAlreadyFirstFocused() {
    return this->alreadyFirstFocused;
}

void View::forceWriteLog(std::string func, std::string line) {
    ...
    logFile.open("/config/gmpack-updater/log.txt", std::ofstream::out | std::ofstream::app);
    if (logFile.is_open()) {
        logFile << ln << std::endl;
    }
    logFile.close();
