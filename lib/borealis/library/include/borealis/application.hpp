...
class Application
{
  public:
    ...
    /**
      * Blocks any and all user touch inputs
      */
    static void blockTouchInputs();
    ...
    /**
      * Unblocks touch inputs after a call to
      * blockTouchInputs()
      */
    static void unblockTouchInputs();
    ...
    static void setCurrentFocus(View* currFocus);
    ...
    static void forceWriteLog(std::string func, std::string line);

    static void setMainView(View* currView);
    static View* getMainView();
    static void setMainViewClassName(std::string name);
	static void setCurrentTouchableView(View* currView);
    static View* getCurrentTouchableView();

    static View* getFocusedElement();

    static unsigned getBlockInputsTokens();
    static unsigned getBlockTouchInputsTokens();

    static int getViewStackSize();

    static bool updateRender(bool additionalTasks = true);
    static void popSplashScreen();
  private:
    //touch
    inline static HidTouchScreenState touchState;
    inline static HidTouchScreenState touchStateOld;
    inline static bool t_moved = false;

    inline static View* mainView;
    inline static std::string mainViewClassName;
    inline static View* currentTouchableView;

    inline static int MAXTAPMOVEMENT = 3;
    //touch
    ...
    inline static unsigned blockTouchInputsTokens = 0; // any value > 0 means inputs are blocked
