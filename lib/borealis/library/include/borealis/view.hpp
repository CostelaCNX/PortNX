...
class View
{
  private:
    ...
    bool alreadyFirstFocused = false;

    //touch
    std::vector<View*> touchableObjects;
    bool clickFocusable = true;
  ...
  public:
    ...
    /**
      * Responsible for informing the user whether a view received focus
      */
    virtual void onWindowFocus();

    /**
      * Responsible for informing the user whether a view lost focus
      */
    virtual void onWindowFocusLost();
    
    //touch
    virtual void processTouch(u32 x, u32 y);
    virtual void processScroll(const ScrollDirection scroll);

    void addTouchableObjects(View* touchableObject);
    int getTouchableObjectsCount();
    std::vector<View*> getTouchableObjects();
    void clearTouchableObjects();
    bool IsPositionInBounds(u32 x, u32 y);
    virtual void click(View* sender);
    void setState(bool state);
    bool isEnabled();

    /**
     * Used for defining whether a touch click
     * should cause this ListItem to be focused
     */
    void setClickFocusable(bool focusable);
    bool isClickFocusable();

    void forceWriteLog(std::string func, std::string line);

    void setFocused(bool focusState);

    void wasAlreadyFirstFocused(bool firstFocused);
    bool isAlreadyFirstFocused();
